...
export interface Study {
 ...
 duration: string; unit: 'minutes' | 'hours'; maxAge: string;
 noUpperAge: boolean; hasEligibility: boolean; eligibility: string; exclusion: string;
 permissions: Record<Permission, boolean>; safeguards: Record<Safeguard, boolean>;
}
export const emptyStudy = (): Study => ({title:'',course:'',fellowName:'',fellowEmail:'',students:[{name:'',email:''}],partner:'',count:'',purpose:'',activity:'',methods:[],otherMethod:'',duration:'',unit:'minutes',maxAge:'',noUpperAge:false,hasEligibility:false,eligibility:'',exclusion:'',permissions:{audio:false,video:false,photos:false,quotes:false,identity:false},safeguards:{distress:false,deception:false,capacity:false,pressure:false}});
