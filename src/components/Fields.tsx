...
export function YesNo({label,value,onChange}:{label:string;value:boolean;onChange:(v:boolean)=>void}){
 const id=useId();return <fieldset className="yesno"><legend>{label}</legend><div className="choices">{[false,true].map(v=><label key={String(v)} className={value===v?'selected':''}><input type="radio" name={id} checked={value===v} onChange={()=>onChange(v)}/>{v?'Yes':'No'}</label>)}</div></fieldset>;
}
