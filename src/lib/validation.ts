...
export function validateDetails(s:Study):FieldErrors {
 ...
 if(!FELLOWS.some(f=>f.name===s.fellowName&&f.email===s.fellowEmail)) e.fellowName='Select a Fellow-in-Charge from the list.';
...
export function validateActivity(s:Study):FieldErrors {
 ...
 if(!integer(s.minAge)||Number(s.minAge)<0)e.minAge='Enter a minimum age as a whole number of zero or greater.';
 if(!s.noUpperAge&&(!integer(s.maxAge)||Number(s.maxAge)<Number(s.minAge)))e.maxAge='Enter a maximum age equal to or above the minimum age.';
