...
export const REVIEW_NOTICE='This study contains circumstances that require review by the Fellow-in-Charge before participant recruitment begins.';
export const REVIEW_CONFIRMATION='This document can be generated, but the study should be reviewed by the Fellow-in-Charge before participant recruitment begins.';
...
export function pisSections(s:Study):{heading:string;paragraphs:string[]}[] {
 ...
 {heading:'Fellow-in-Charge and Student Investigator(s)',paragraphs:['Fellow-in-Charge:',contact({name:s.fellowName,email:s.fellowEmail}),'Student Investigators:',...s.students.map(p=>contact(p))]},
 ...
 {heading:'Who can participate in the study?',paragraphs:[s.noUpperAge?`Participants will be aged ${Number(s.minAge)} years or older, with no specific upper age limit.`:`Participants will be between the ages of ${Number(s.minAge)} and ${Number(s.maxAge)} years old, inclusive.`,...(s.hasEligibility?[s.eligibility.trim()]:[])]},
 ...
 {heading:'Whom should I contact if I have questions or concerns?',paragraphs:['For questions about this study, please contact:','Fellow-in-Charge:',contact({name:s.fellowName,email:s.fellowEmail},false),'Student Investigators:',...s.students.map(p=>contact(p,false))]},
 ];
}
